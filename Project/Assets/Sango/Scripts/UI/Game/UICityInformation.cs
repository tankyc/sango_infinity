...
using Sango.Core; namespace Sango.UI
{
    public class UICityInformation : UGUIWindow
    {
        ...
        public void Show(City city)
        {
            ...
            {
                // TODO: 皇帝
                emperorLabel.text = city.mCity.Name;
            }
            switch (showTab)
            {
                case 0:
        ...
        void UpdateItemsContent()
        {
            ...
            items_inited = true;
            itemPool.Reset();
            List<ItemType> ItemTypes = Target.mForce.createdItemTypes;
            int len = ItemTypes.Count;
            for (int i = 0; i < len; i++)
            {
                ItemType itemType = ItemTypes[i];
                int totalNum = Target.itemStore.GetNumber(itemType);
        ...
        void UpdateBuildingsContent()
        {
            ...
            {
                if (x.IsIntrior && x.level == 1 && x.IsValid(Target.mForce) && x.canBuild)
                {
