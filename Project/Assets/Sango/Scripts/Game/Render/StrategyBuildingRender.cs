...
{
    public class StrategyBuildingRender : ObjectRender
    {
        StrategyBuilding Building { get; set; }
        BuildingModel BuildingModel { get; set; }
        UGUIWindow HeadBar { get; set; }
        bool isComplate = false;
        bool isUpgrading = false;

        public StrategyBuildingRender()
        {

        }

        public StrategyBuildingRender(StrategyBuilding building)
        {
            Owener = building;
            Building = building;
            MapObject = MapObject.Create($"{Building.mCity.Name}-{Building.Name}");
        ...
        public void OnModelLoaded(GameObject obj)
        {
            ...
            GameObject headBar = PoolManager.Create(GameRenderHelper.BuildingHeadbarRes, (x) =>
            {
                GameSetting.Instance.UpdateTargetGameObject(x);
