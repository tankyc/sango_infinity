...
namespace Sango.Game.Condition
{
    ...
    {
        public delegate int PersonAttributeGetCall(Person city);

        public int value;
        public int compareResult; //-2小于等于-1小于0等于1大于2大于等于
        public int cityId;  //0则不需要检测
        public PersonAttributeGetCall getCall;
        public override ConditionType ConditionType { get { return ConditionType.PersonAttributeCheck; } }

        public override bool Check(ConditionParams sanObj)
        {
            Person target = sanObj.Person; 
        ...
        {
            return new PersonAttributeCheck()
            {
                value = value,
