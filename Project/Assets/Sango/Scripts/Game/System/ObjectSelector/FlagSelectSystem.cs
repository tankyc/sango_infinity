...
{
    /// <summary>
    /// 旗帜选择系统，用于在通用对象选择窗口中选择旗帜
    /// </summary>
    [GameSystem]
    public class FlagSelectSystem : ObjectSelectSystem, IObjectSelectSystem<Flag>
    {
        /// <summary>
        /// 通用对象选择接口实现 - 供UIDataEdit按数据集类型统一调用
        /// </summary>
        void IObjectSelectSystem<Flag>.Start(List<Flag> candidates, List<Flag> resultList, int limit, Action<List<Flag>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName)
        {
        ...
        /// 默认排序标题组名称
        /// </summary>
        public string defualtTitleName = "旗帜";

        /// <summary>
        /// 默认排序标题列表
        /// </summary>
        public List<ObjectSortTitle> defualtTitleList = FlagSortFunction.DefaultSortList;
        ...
        /// 启动旗帜选择系统
        /// </summary>
        /// <param name="flags">候选旗帜列表</param>
        /// <param name="resultList">已选中的旗帜列表</param>
        ...
        public void Start(List<Flag> flags, List<Flag> resultList, int limit, Action<List<Flag>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName, int sortIndex = 1)
        {
            donotFinishThisSystem = false;
            selectLimit = Math.Min(limit, flags.Count);
            Objects = new List<SangoObject>(flags);
            finishAction = action;
