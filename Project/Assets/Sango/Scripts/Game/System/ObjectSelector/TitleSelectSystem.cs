...
{
    /// <summary>
    /// 爵位称号选择系统，用于在通用对象选择窗口中选择爵位称号
    /// </summary>
    [GameSystem]
    public class TitleSelectSystem : ObjectSelectSystem, IObjectSelectSystem<Title>
    {
        /// <summary>
        /// 通用对象选择接口实现 - 供UIDataEdit按数据集类型统一调用
        /// </summary>
        void IObjectSelectSystem<Title>.Start(List<Title> candidates, List<Title> resultList, int limit, Action<List<Title>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName)
        {
        ...
        /// 选择完成后的回调
        /// </summary>
        Action<List<Title>> finishAction;
        ...
        /// 默认排序标题组名称
        /// </summary>
        public string defualtTitleName = "爵位";

        /// <summary>
        /// 默认排序标题列表
        /// </summary>
        public List<ObjectSortTitle> defualtTitleList = TitleSortFunction.DefaultSortList;
        ...
        /// 启动爵位称号选择系统
        /// </summary>
        /// <param name="titles">候选爵位称号列表</param>
        /// <param name="resultList">已选中的爵位称号列表</param>
        ...
        public void Start(List<Title> titles, List<Title> resultList, int limit, Action<List<Title>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName, int sortIndex = 1)
        {
            donotFinishThisSystem = false;
            selectLimit = Math.Min(limit, titles.Count);
            Objects = new List<SangoObject>(titles);
            finishAction = action;
