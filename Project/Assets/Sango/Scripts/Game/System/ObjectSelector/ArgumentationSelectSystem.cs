...
{
    /// <summary>
    /// 义理选择系统，用于在通用对象选择窗口中选择义理
    /// </summary>
    [GameSystem]
    public class ArgumentationSelectSystem : ObjectSelectSystem, IObjectSelectSystem<Argumentation>
    {
        /// <summary>
        /// 通用对象选择接口实现 - 供UIDataEdit按数据集类型统一调用
        /// </summary>
        void IObjectSelectSystem<Argumentation>.Start(List<Argumentation> candidates, List<Argumentation> resultList, int limit, Action<List<Argumentation>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName)
        {
        ...
        /// 选择完成后的回调
        /// </summary>
        Action<List<Argumentation>> finishAction;
        ...
        /// 默认排序标题组名称
        /// </summary>
        public string defualtTitleName = "义理";

        /// <summary>
        /// 默认排序标题列表
        /// </summary>
        public List<ObjectSortTitle> defualtTitleList = ArgumentationSortFunction.DefaultSortList;
        ...
        /// 启动义理选择系统
        /// </summary>
        /// <param name="argumentations">候选义理列表</param>
        /// <param name="resultList">已选中的义理列表</param>
        ...
        public void Start(List<Argumentation> argumentations, List<Argumentation> resultList, int limit, Action<List<Argumentation>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName, int sortIndex = 1)
        {
            donotFinishThisSystem = false;
            selectLimit = Math.Min(limit, argumentations.Count);
            Objects = new List<SangoObject>(argumentations);
            finishAction = action;
