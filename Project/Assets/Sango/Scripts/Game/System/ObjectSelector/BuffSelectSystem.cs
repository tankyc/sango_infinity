...
{
    /// <summary>
    /// Buff选择系统，用于在通用对象选择窗口中选择Buff
    /// </summary>
    [GameSystem]
    public class BuffSelectSystem : ObjectSelectSystem, IObjectSelectSystem<Buff>
    {
        /// <summary>
        /// 通用对象选择接口实现 - 供UIDataEdit按数据集类型统一调用
        /// </summary>
        void IObjectSelectSystem<Buff>.Start(List<Buff> candidates, List<Buff> resultList, int limit, Action<List<Buff>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName)
        {
        ...
        /// 默认排序标题组名称
        /// </summary>
        public string defualtTitleName = "状态";

        /// <summary>
        /// 默认排序标题列表
        /// </summary>
        public List<ObjectSortTitle> defualtTitleList = BuffSortFunction.DefaultSortList;
        ...
        /// 启动Buff选择系统
        /// </summary>
        /// <param name="buffs">候选Buff列表</param>
        /// <param name="resultList">已选中的Buff列表</param>
        ...
        public void Start(List<Buff> buffs, List<Buff> resultList, int limit, Action<List<Buff>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName, int sortIndex = 1)
        {
            donotFinishThisSystem = false;
            selectLimit = Math.Min(limit, buffs.Count);
            Objects = new List<SangoObject>(buffs);
            finishAction = action;
