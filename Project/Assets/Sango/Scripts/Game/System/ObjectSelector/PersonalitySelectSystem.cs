...
{
    /// <summary>
    /// 性格选择系统，用于在通用对象选择窗口中选择性格
    /// </summary>
    [GameSystem]
    public class PersonalitySelectSystem : ObjectSelectSystem, IObjectSelectSystem<Personality>
    {
        /// <summary>
        /// 通用对象选择接口实现 - 供UIDataEdit按数据集类型统一调用
        /// </summary>
        void IObjectSelectSystem<Personality>.Start(List<Personality> candidates, List<Personality> resultList, int limit, Action<List<Personality>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName)
        {
        ...
        /// 选择完成后的回调
        /// </summary>
        Action<List<Personality>> finishAction;
        ...
        /// 默认排序标题组名称
        /// </summary>
        public string defualtTitleName = "性格";

        /// <summary>
        /// 默认排序标题列表
        /// </summary>
        public List<ObjectSortTitle> defualtTitleList = PersonalitySortFunction.DefaultSortList;
        ...
        /// 启动性格选择系统
        /// </summary>
        /// <param name="personalities">候选性格列表</param>
        /// <param name="resultList">已选中的性格列表</param>
        ...
        public void Start(List<Personality> personalities, List<Personality> resultList, int limit, Action<List<Personality>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName, int sortIndex = 1)
        {
            donotFinishThisSystem = false;
            selectLimit = Math.Min(limit, personalities.Count);
            Objects = new List<SangoObject>(personalities);
            finishAction = action;
