...
{
    /// <summary>
    /// 工作类型选择系统，用于在通用对象选择窗口中选择工作类型
    /// </summary>
    [GameSystem]
    public class JobTypeSelectSystem : ObjectSelectSystem, IObjectSelectSystem<JobType>
    {
        /// <summary>
        /// 通用对象选择接口实现 - 供UIDataEdit按数据集类型统一调用
        /// </summary>
        void IObjectSelectSystem<JobType>.Start(List<JobType> candidates, List<JobType> resultList, int limit, Action<List<JobType>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName)
        {
        ...
        /// 选择完成后的回调
        /// </summary>
        Action<List<JobType>> finishAction;
        ...
        /// 默认排序标题组名称
        /// </summary>
        public string defualtTitleName = "工作";

        /// <summary>
        /// 默认排序标题列表
        /// </summary>
        public List<ObjectSortTitle> defualtTitleList = JobTypeSortFunction.DefaultSortList;
        ...
        /// 启动工作类型选择系统
        /// </summary>
        /// <param name="jobTypes">候选工作类型列表</param>
        /// <param name="resultList">已选中的工作类型列表</param>
        ...
        public void Start(List<JobType> jobTypes, List<JobType> resultList, int limit, Action<List<JobType>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName, int sortIndex = 1)
        {
            donotFinishThisSystem = false;
            selectLimit = Math.Min(limit, jobTypes.Count);
            Objects = new List<SangoObject>(jobTypes);
            finishAction = action;
