...
{
    /// <summary>
    /// 武将等级选择系统，用于在通用对象选择窗口中选择武将等级
    /// </summary>
    [GameSystem]
    public class PersonLevelSelectSystem : ObjectSelectSystem, IObjectSelectSystem<PersonLevel>
    {
        /// <summary>
        /// 通用对象选择接口实现 - 供UIDataEdit按数据集类型统一调用
        /// </summary>
        void IObjectSelectSystem<PersonLevel>.Start(List<PersonLevel> candidates, List<PersonLevel> resultList, int limit, Action<List<PersonLevel>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName)
        {
        ...
        /// 选择完成后的回调
        /// </summary>
        Action<List<PersonLevel>> finishAction;
        ...
        /// 默认排序标题组名称
        /// </summary>
        public string defualtTitleName = "武将等级";

        /// <summary>
        /// 默认排序标题列表
        /// </summary>
        public List<ObjectSortTitle> defualtTitleList = PersonLevelSortFunction.DefaultSortList;
        ...
        /// 启动武将等级选择系统
        /// </summary>
        /// <param name="personLevels">候选武将等级列表</param>
        /// <param name="resultList">已选中的武将等级列表</param>
        ...
        public void Start(List<PersonLevel> personLevels, List<PersonLevel> resultList, int limit, Action<List<PersonLevel>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName, int sortIndex = 1)
        {
            donotFinishThisSystem = false;
            selectLimit = Math.Min(limit, personLevels.Count);
            Objects = new List<SangoObject>(personLevels);
            finishAction = action;
