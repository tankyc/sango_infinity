...
{
    /// <summary>
    /// 能力变化类型选择系统，用于在通用对象选择窗口中选择能力变化类型
    /// </summary>
    [GameSystem]
    public class AttributeChangeTypeSelectSystem : ObjectSelectSystem, IObjectSelectSystem<AttributeChangeType>
    {
        /// <summary>
        /// 通用对象选择接口实现 - 供UIDataEdit按数据集类型统一调用
        /// </summary>
        void IObjectSelectSystem<AttributeChangeType>.Start(List<AttributeChangeType> candidates, List<AttributeChangeType> resultList, int limit, Action<List<AttributeChangeType>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName)
        {
        ...
        /// 选择完成后的回调
        /// </summary>
        Action<List<AttributeChangeType>> finishAction;
        ...
        /// 默认排序标题组名称
        /// </summary>
        public string defualtTitleName = "能力变化";

        /// <summary>
        /// 默认排序标题列表
        /// </summary>
        public List<ObjectSortTitle> defualtTitleList = AttributeChangeTypeSortFunction.DefaultSortList;
        ...
        /// 启动能力变化类型选择系统
        /// </summary>
        /// <param name="attributeChangeTypes">候选能力变化类型列表</param>
        /// <param name="resultList">已选中的能力变化类型列表</param>
        ...
        public void Start(List<AttributeChangeType> attributeChangeTypes, List<AttributeChangeType> resultList, int limit, Action<List<AttributeChangeType>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName, int sortIndex = 1)
        {
            donotFinishThisSystem = false;
            selectLimit = Math.Min(limit, attributeChangeTypes.Count);
            Objects = new List<SangoObject>(attributeChangeTypes);
            finishAction = action;
        ...
        /// <summary>
        /// 确认选择回调，将通用对象列表转换为能力变化类型列表
        /// </summary>
        /// <param name="objects">已选中的通用对象列表</param>
        public void OnBaseSure(List<SangoObject> objects)
        {
            List<AttributeChangeType> result = new List<AttributeChangeType>();
            foreach (SangoObject obj in objects)
            {
                result.Add((AttributeChangeType)obj);
            }
