...
{
    /// <summary>
    /// 科技选择系统，用于在通用对象选择窗口中选择科技
    /// </summary>
    [GameSystem]
    public class TechniqueSelectSystem : ObjectSelectSystem, IObjectSelectSystem<Technique>
    {
        /// <summary>
        /// 通用对象选择接口实现 - 供UIDataEdit按数据集类型统一调用
        /// </summary>
        void IObjectSelectSystem<Technique>.Start(List<Technique> candidates, List<Technique> resultList, int limit, Action<List<Technique>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName)
        {
        ...
        /// 选择完成后的回调
        /// </summary>
        Action<List<Technique>> finishAction;
        ...
        /// 默认排序标题组名称
        /// </summary>
        public string defualtTitleName = "科技";

        /// <summary>
        /// 默认排序标题列表
        /// </summary>
        public List<ObjectSortTitle> defualtTitleList = TechniqueSortFunction.DefaultSortList;
        ...
        /// 启动科技选择系统
        /// </summary>
        /// <param name="techniques">候选科技列表</param>
        /// <param name="resultList">已选中的科技列表</param>
        ...
        public void Start(List<Technique> techniques, List<Technique> resultList, int limit, Action<List<Technique>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName, int sortIndex = 1)
        {
            donotFinishThisSystem = false;
            selectLimit = Math.Min(limit, techniques.Count);
            Objects = new List<SangoObject>(techniques);
            finishAction = action;
