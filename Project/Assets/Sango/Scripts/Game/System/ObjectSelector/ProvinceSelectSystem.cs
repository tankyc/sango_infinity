...
{
    /// <summary>
    /// 州选择系统，用于在通用对象选择窗口中选择州
    /// </summary>
    [GameSystem]
    public class ProvinceSelectSystem : ObjectSelectSystem, IObjectSelectSystem<Province>
    {
        /// <summary>
        /// 通用对象选择接口实现 - 供UIDataEdit按数据集类型统一调用
        /// </summary>
        void IObjectSelectSystem<Province>.Start(List<Province> candidates, List<Province> resultList, int limit, Action<List<Province>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName)
        {
        ...
        /// 选择完成后的回调
        /// </summary>
        Action<List<Province>> finishAction;
        ...
        /// 默认排序标题组名称
        /// </summary>
        public string defualtTitleName = "州";

        /// <summary>
        /// 默认排序标题列表
        /// </summary>
        public List<ObjectSortTitle> defualtTitleList = ProvinceSortFunction.DefaultSortList;
        ...
        /// 启动州选择系统
        /// </summary>
        /// <param name="provinces">候选州列表</param>
        /// <param name="resultList">已选中的州列表</param>
        ...
        public void Start(List<Province> provinces, List<Province> resultList, int limit, Action<List<Province>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName, int sortIndex = 1)
        {
            donotFinishThisSystem = false;
            selectLimit = Math.Min(limit, provinces.Count);
            Objects = new List<SangoObject>(provinces);
            finishAction = action;
