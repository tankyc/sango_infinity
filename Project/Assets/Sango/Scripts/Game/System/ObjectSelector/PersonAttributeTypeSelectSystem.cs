...
{
    /// <summary>
    /// 武将属性类型选择系统，用于在通用对象选择窗口中选择武将属性类型
    /// </summary>
    [GameSystem]
    public class PersonAttributeTypeSelectSystem : ObjectSelectSystem, IObjectSelectSystem<PersonAttributeType>
    {
        /// <summary>
        /// 通用对象选择接口实现 - 供UIDataEdit按数据集类型统一调用
        /// </summary>
        void IObjectSelectSystem<PersonAttributeType>.Start(List<PersonAttributeType> candidates, List<PersonAttributeType> resultList, int limit, Action<List<PersonAttributeType>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName)
        {
        ...
        /// 选择完成后的回调
        /// </summary>
        Action<List<PersonAttributeType>> finishAction;
        ...
        /// 默认排序标题组名称
        /// </summary>
        public string defualtTitleName = "属性";

        /// <summary>
        /// 默认排序标题列表
        /// </summary>
        public List<ObjectSortTitle> defualtTitleList = PersonAttributeTypeSortFunction.DefaultSortList;
        ...
        /// 启动武将属性类型选择系统
        /// </summary>
        /// <param name="personAttributeTypes">候选武将属性类型列表</param>
        /// <param name="resultList">已选中的武将属性类型列表</param>
        ...
        public void Start(List<PersonAttributeType> personAttributeTypes, List<PersonAttributeType> resultList, int limit, Action<List<PersonAttributeType>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName, int sortIndex = 1)
        {
            donotFinishThisSystem = false;
            selectLimit = Math.Min(limit, personAttributeTypes.Count);
            Objects = new List<SangoObject>(personAttributeTypes);
            finishAction = action;
        ...
        /// <summary>
        /// 确认选择回调，将通用对象列表转换为武将属性类型列表
        /// </summary>
        /// <param name="objects">已选中的通用对象列表</param>
        public void OnBaseSure(List<SangoObject> objects)
        {
            List<PersonAttributeType> result = new List<PersonAttributeType>();
            foreach (SangoObject obj in objects)
            {
                result.Add((PersonAttributeType)obj);
            }
