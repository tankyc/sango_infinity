...
{
    /// <summary>
    /// 兵种类型选择系统，用于在通用对象选择窗口中选择兵种类型
    /// </summary>
    [GameSystem]
    public class TroopTypeSelectSystem : ObjectSelectSystem, IObjectSelectSystem<TroopType>
    {
        /// <summary>
        /// 通用对象选择接口实现 - 供UIDataEdit按数据集类型统一调用
        /// </summary>
        void IObjectSelectSystem<TroopType>.Start(List<TroopType> candidates, List<TroopType> resultList, int limit, Action<List<TroopType>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName)
        {
        ...
        /// 选择完成后的回调
        /// </summary>
        Action<List<TroopType>> finishAction;
        ...
        /// 默认排序标题组名称
        /// </summary>
        public string defualtTitleName = "兵种";

        /// <summary>
        /// 默认排序标题列表
        /// </summary>
        public List<ObjectSortTitle> defualtTitleList = TroopTypeSortFunction.DefaultSortList;
        ...
        /// 启动兵种类型选择系统
        /// </summary>
        /// <param name="troopTypes">候选兵种类型列表</param>
        /// <param name="resultList">已选中的兵种类型列表</param>
        ...
        public void Start(List<TroopType> troopTypes, List<TroopType> resultList, int limit, Action<List<TroopType>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName, int sortIndex = 1)
        {
            donotFinishThisSystem = false;
            selectLimit = Math.Min(limit, troopTypes.Count);
            Objects = new List<SangoObject>(troopTypes);
            finishAction = action;
