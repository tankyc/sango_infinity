...
{
    /// <summary>
    /// 地形类型选择系统，用于在通用对象选择窗口中选择地形类型
    /// </summary>
    [GameSystem]
    public class TerrainTypeSelectSystem : ObjectSelectSystem, IObjectSelectSystem<TerrainType>
    {
        /// <summary>
        /// 通用对象选择接口实现 - 供UIDataEdit按数据集类型统一调用
        /// </summary>
        void IObjectSelectSystem<TerrainType>.Start(List<TerrainType> candidates, List<TerrainType> resultList, int limit, Action<List<TerrainType>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName)
        {
        ...
        /// 选择完成后的回调
        /// </summary>
        Action<List<TerrainType>> finishAction;
        ...
        /// 默认排序标题组名称
        /// </summary>
        public string defualtTitleName = "地形";

        /// <summary>
        /// 默认排序标题列表
        /// </summary>
        public List<ObjectSortTitle> defualtTitleList = TerrainTypeSortFunction.DefaultSortList;
        ...
        /// 启动地形类型选择系统
        /// </summary>
        /// <param name="terrainTypes">候选地形类型列表</param>
        /// <param name="resultList">已选中的地形类型列表</param>
        ...
        public void Start(List<TerrainType> terrainTypes, List<TerrainType> resultList, int limit, Action<List<TerrainType>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName, int sortIndex = 1)
        {
            donotFinishThisSystem = false;
            selectLimit = Math.Min(limit, terrainTypes.Count);
            Objects = new List<SangoObject>(terrainTypes);
            finishAction = action;
