...
{
    /// <summary>
    /// 官职选择系统，用于在通用对象选择窗口中选择官职
    /// </summary>
    [GameSystem]
    public class OfficialSelectSystem : ObjectSelectSystem, IObjectSelectSystem<Official>
    {
        /// <summary>
        /// 通用对象选择接口实现 - 供UIDataEdit按数据集类型统一调用
        /// </summary>
        void IObjectSelectSystem<Official>.Start(List<Official> candidates, List<Official> resultList, int limit, Action<List<Official>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName)
        {
        ...
        /// 选择完成后的回调
        /// </summary>
        Action<List<Official>> finishAction;
        ...
        /// 默认排序标题组名称
        /// </summary>
        public string defualtTitleName = "官职";

        /// <summary>
        /// 默认排序标题列表
        /// </summary>
        public List<ObjectSortTitle> defualtTitleList = OfficialSortFunction.DefaultSortList;
        ...
        /// 启动官职选择系统
        /// </summary>
        /// <param name="officials">候选官职列表</param>
        /// <param name="resultList">已选中的官职列表</param>
        ...
        public void Start(List<Official> officials, List<Official> resultList, int limit, Action<List<Official>> action, List<ObjectSortTitle> customSortTitles, string cutomSortTitleName, int sortIndex = 1)
        {
            donotFinishThisSystem = false;
            selectLimit = Math.Min(limit, officials.Count);
            Objects = new List<SangoObject>(officials);
            finishAction = action;
