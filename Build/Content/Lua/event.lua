...
_xpcall.__call = function(self, ...)	
	...
	--
	--	if nil == self.obj then
	--		local func = function() self.func(unpack(args, 1, #(args))) end
	--		return xpcall(func, traceback)
	--	else
	--		local func = function() self.func(self.obj, unpack(args, 1, #(args))) end
	--		return xpcall(func, traceback)
	--	end
	--end
end
...
_xpcall.__eq = function(lhs, rhs)
	...
	if rawequal(lhs, rhs) then
		...
		if not rawequal(_xpcall,metaTable_va) then
			return false
		end

		local metaTable_vb=getmetatable(rhs)
		if not rawequal(_xpcall,metaTable_vb) then
			return false
		end
...
_pcall.__eq = function(lhs, rhs)
	...
	if rawequal(lhs, rhs) then
		...
		local metaTable_va=getmetatable(lhs)
		if not rawequal(_pcall,metaTable_va) then
			return false
		end

		local metaTable_vb=getmetatable(rhs)
		if not rawequal(_pcall,metaTable_vb) then
			return false
		end
